...
class UserModelTestCase(unittest.TestCase):
    ...
    def test_valid_email_change_token(self):
        u = User(email='john@qq.com', password='1234')
        db.session.add(u)
        db.session.commit()
        token = u.generate_email_token('susan@qq.com')
        self.assertFalse(u.change_email(token))
        self.assertTrue(u.email == 'susan@qq.com')

    def test_invalid_email_change_token(self):
        u1 = User(email='john@qq.com', password='cat')
        u2 = User(email='susan@qq.com', password='dog')
        db.session.add_all([u1, u2])
        db.session.commit()
        token = u1.generate_email_token('david@qq.com')
        self.assertFalse(u2.change_email(token))
        self.assertTrue(u2.email == 'susan@qq.com')
    ...
    def test_duplicate_email_change_token(self):
        ...
        db.session.add(u1)
        db.session.add(u2)
        token = u2.generate_email_token('john@qq.com')
        self.assertFalse(u2.change_email(token))
        self.assertTrue(u2.email == 'susan@qq.com')

    def test_roles_and_permission(self):
        Role.insert_roles()
        u = User(email='1053571092@qq.com', password='1234')
    ...
    def test_timestamps(self):
        ...
        time.sleep(3)
        self.assertTrue(
            (datetime.utcnow() - u.last_time).total_seconds() > 3)
    ...
    def test_ping(self):
        ...
        db.session.commit()
        time.sleep(2)
        last_seen_before = u.last_time
        u.ping()
        self.assertTrue(u.last_time > last_seen_before)
